...
void Block::unserialize(DataStream &s, HotStuffCore *hsc) {
    ...
    s >> n;
    n = letoh(n);
    HOTSTUFF_LOG_INFO("[[Block Serialize : graph size = %d]]", n);
    uint256_t key;
    uint32_t set_size;
    ...
    for(int key_i=0; key_i<n; key_i++){
        ...
        HOTSTUFF_LOG_INFO("[[Block Serialize : graph key = %s]]", get_hex(key).c_str());
        s >> set_size;
        set_size = letoh(set_size);
        HOTSTUFF_LOG_INFO("[[Block Serialize : graph key-set size = %d]]", set_size);
        for(int i=0; i<set_size; i++){
            s >> set_element;
            graph[key].insert(set_element);
            HOTSTUFF_LOG_INFO("[[Block Serialize : graph set val = %s]]", get_hex(set_element).c_str());
        }
    }
