...
//                try_release_cmd(cmd);
    ...
    std::vector<std::pair<uint256_t, uint256_t>> get_updated_missing_edges() {
        ...
        HOTSTUFF_LOG_INFO("[[get_updated_missing_edges]] [R-] [L-] local_order_seen_execute_level_cache size = %d", local_order_seen_execute_level_cache->get_size());
        ...
        for(auto it_1=local_order_seen_execute_level_cache->begin(); it_1!=local_order_seen_execute_level_cache->end(); it_1++) {
            ...
            for(auto it_2=it_1.next(); it_2!=local_order_seen_execute_level_cache->end(); it_2++) {
                ...
                HOTSTUFF_LOG_INFO("[[get_updated_missing_edges]] [R-] [L-] local_order_seen_execute_level_cache edge = %.10s -> %.10s", get_hex(from_v).c_str(), get_hex(to_v).c_str());
