...
int main(int argc, char **argv) {
    ...
    auto opt_sb_users = Config::OptValInt::create(100);
    auto opt_sb_prob_choose_mtx = Config::OptValDouble::create(1.0);
    auto opt_sb_skew_factor = Config::OptValDouble::create(0.99);
    auto opt_fairness_parameter = Config::OptValDouble::create(1.0);  // Themis
    auto opt_blk_size = Config::OptValInt::create(400);
...
void HotStuffApp::client_request_cmd_handler(MsgReqCmd &&msg, const conn_t &conn) {
    ...
    auto dependency = small_bank_manager->get_users_detail(cmd->get_payload());

    exec_command(cmd_hash, dependency, [this, addr, cmd](Finality fin) {
        ...
        //     HOTSTUFF_LOG_INFO("ABCDEFGHIJKLMNOPQRSTUVWXYZ ABCDEFGHIJKLMNOPQRSTUVWXYZ");
        // }
        

        // et.stop();
        // HOTSTUFF_LOG_INFO("Busy waiting time at command execution = %lf", et.elapsed_sec);
